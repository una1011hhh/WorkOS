  ...
  async load(): Promise<WorkData> {
    ...
    const mappedTasks: Task[] = (tasks.data ?? []).map(row => {
      ...
      return {
        ...
        dueDate: row.due_date ?? "",
        plannedStart: (row as any).planned_start ? toDateTimeLocal((row as any).planned_start) : undefined,
        plannedEnd: (row as any).planned_end ? toDateTimeLocal((row as any).planned_end) : undefined,
