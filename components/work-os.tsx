...
type PlannerEvent = { id: string; title: string; start: number; end: number; kind: "meeting" | "planned"; detail: string; task?: Task; lane?: number; lanes?: number };
...
const layoutPlannerEvents = (source: PlannerEvent[]) => (["meeting","planned"] as const).flatMap(kind => {
  const events=source.filter(event=>event.kind===kind).map(event=>({...event})).sort((a,b)=>a.start-b.start||a.end-b.end),ends:number[]=[];
  events.forEach(event=>{let lane=ends.findIndex(end=>end<=event.start);if(lane<0)lane=ends.length;ends[lane]=event.end;event.lane=lane});
...
function DailyPlanner({data,setData,onTask,notify}:{data:WorkData;setData:React.Dispatch<React.SetStateAction<WorkData>>;onTask:(task:Task)=>void;notify:(message:string)=>void}) {
  ...
  const events=layoutPlannerEvents([...meetingEvents,...plannedEvents].filter(event=>event.end>8*60&&event.start<20*60));
  const busy=[...meetingEvents,...plannedEvents].sort((a,b)=>a.start-b.start);
  const busyMinutes=Array.from({length:24},(_,index)=>8*60+index*30).filter(minute=>busy.some(event=>event.start<minute+30&&event.end>minute)).length*30;
  ...
  const dropPlannedTask=(event:React.DragEvent<HTMLDivElement>)=>{event.preventDefault();const taskId=draggedTaskRef.current,task=data.tasks.find(item=>item.id===taskId);if(!task)return;const rect=event.currentTarget.getBoundingClientRect(),raw=8*60+(event.clientY-rect.top),begin=Math.max(8*60,Math.min(20*60-15,Math.round(raw/15)*15)),oldStart=plannerMinutes(task.plannedStart),oldEnd=plannerMinutes(task.plannedEnd),minutes=Math.max(15,oldEnd>oldStart?oldEnd-oldStart:Math.round(Math.max(.25,task.estimatedHours||1)*60));setData(current=>({...current,tasks:current.tasks.map(item=>item.id===taskId?{...item,plannedStart:plannerDateTime(date,begin),plannedEnd:plannerDateTime(date,Math.min(20*60,begin+minutes))}:item)}));draggedTaskRef.current="";notify(`已移动到 ${plannerTime(begin)}`)};
  ...
  return <div className="planner-page">
    ...
    <div className="planner-summary"><span>会议 <b>{meetingEvents.length}</b></span><span>计划任务 <b>{plannedEvents.length}</b></span><span>可安排时间 <b>{Math.max(0,(12*60-busyMinutes)/60).toFixed(1)}h</b></span></div>
    <div className="planner-layout">
      <section className="panel planner-calendar"><div className="planner-legend"><span className="meeting">会议</span><span className="planned">计划任务</span><small>任务可上下拖动；重叠事项会自动缩窄并排</small></div><div className="planner-track-head"><span>会议</span><span>计划任务</span></div><div className="planner-timeline"><div className="planner-hour-labels">{Array.from({length:13},(_,index)=><span key={index} style={{top:`${index*60}px`}}>{plannerTime((8+index)*60)}</span>)}</div><div className="planner-canvas" onDragOver={event=>event.preventDefault()} onDrop={dropPlannedTask}>{Array.from({length:13},(_,index)=><i className="planner-grid-line" key={index} style={{top:`${index*60}px`}}/>)}<i className="planner-lane-divider"/>{events.map(event=>{const startMinute=Math.max(8*60,event.start),endMinute=Math.min(20*60,event.end),lanes=event.lanes||1,lane=event.lane||0,trackStart=event.kind==="meeting"?0:50,trackWidth=50;return <button key={event.id} draggable={event.kind==="planned"} className={`planner-event ${event.kind}`} style={{top:`${startMinute-8*60}px`,height:`${Math.max(24,endMinute-startMinute)}px`,left:`calc(${trackStart+lane/lanes*trackWidth}% + 4px)`,width:`calc(${trackWidth/lanes}% - 8px)`}} onDragStart={()=>{if(event.task){draggedTaskRef.current=event.task.id;didDragRef.current=true}}} onDragEnd={()=>window.setTimeout(()=>{didDragRef.current=false},0)} onClick={()=>{if(!didDragRef.current&&event.task)onTask(event.task)}}><strong>{event.title}</strong><span>{plannerTime(event.start)}–{plannerTime(event.end)}</span><small>{event.detail}</small></button>})}</div></div></section>
...
function DashboardDetailsDrawer({ kind, data, suggestedTask, focusReason, todayTasks, timeline, riskGroups, projects, insights, onClose, onTask, setView }: { kind: DashboardDetailKind | null; data: WorkData; suggestedTask?: Task; focusReason: string; todayTasks: Task[]; timeline: { id: string; kind: "meeting" | "task"; time: string; title: string; detail: string; onClick: () => void }[]; riskGroups: { title: string; rows: Task[] }[]; projects: { project: Project; tasks: Task[]; progress: { progress: number; completed: number; total: number }; risks: number }[]; insights: string[]; onClose: () => void; onTask: (t: Task) => void; setView: (v: View) => void }) {
  ...
  return <DrillDownDrawer key={kind || "closed"} open={!!kind} onClose={onClose} title={title} subtitle="点击记录可进入对应详情">
