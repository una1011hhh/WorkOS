...
function ContactCenter({data,query,onSaveContact,onDeleteContact,onSaveGroup,onDeleteGroup}:{data:WorkData;query:string;onSaveContact:(c:Contact)=>void;onDeleteContact:(c:Contact)=>void;onSaveGroup:(g:ContactGroup)=>void;onDeleteGroup:(g:ContactGroup)=>void}) {
  ...
  const blankContact=():Contact=>({id:uid("contact"),name:"",role:"",team:"",company:"",email:"",phone:"",notes:"",createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()});
  const blankGroup=():ContactGroup=>({id:uid("group"),name:"",description:"",contactIds:[],createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()});
...
function Field({label,children,wide,helper,tip}:{label:string;children:React.ReactNode;wide?:boolean;helper?:string;tip?:string}){return <label className={cn("field",wide&&"wide")}><span className="field-label">{label}{tip&&<button type="button" className="field-help" aria-label={`${label}说明`} title={tip} onClick={e=>e.preventDefault()}>?</button>}</span>{children}{helper&&<small className="field-helper">{helper}</small>}</label>}
...
function CaptureDialog({open,onOpenChange,onAdd}:{open:boolean;onOpenChange:(o:boolean)=>void;onAdd:(t:Task)=>void}) {
  ...
  const submit=()=>{if(!title.trim())return;onAdd({id:uid("task"),title,description:"",source,requester:requester||"自己",projectId:"",status:"Inbox",priority:"P2",dueDate:"",estimatedHours:.5,actualHours:0,createdAt:todayISO(),tags:[],notes:"",waitingFor:"",timeTracking:blankTracking()});setTitle("");onOpenChange(false)};
...
function TaskDialog({open,task,projects,onCreateProject,onOpenChange,onSave}:{open:boolean;task:Task|null;projects:Project[];onCreateProject:(p:Project)=>Project;onOpenChange:(o:boolean)=>void;onSave:(t:Task)=>void}) {
  const blank=():Task=>({id:uid("task"),title:"",description:"",source:"手动创建",requester:"自己",projectId:"",status:"Todo",priority:"P1",dueDate:addDays(new Date(),2).toISOString().slice(0,10),estimatedHours:1,actualHours:0,createdAt:todayISO(),tags:[],notes:"",waitingFor:"",timeTracking:blankTracking()});
  const [form,setForm]=useState<Task>(blank());
  useEffect(()=>{if(open)setForm(task?{...task,tags:[...task.tags],timeTracking:task.timeTracking||blankTracking(),actualHours:taskHours(task)}:blank())},[open,task]);
  const f=<K extends keyof Task>(k:K,v:Task[K])=>setForm(x=>({...x,[k]:v}));
  return <BaseDialog open={open} onOpenChange={onOpenChange} title={task?"编辑任务":"新建任务"} subtitle="补全上下文，未来的你会感谢现在的你。" wide>
    <div className="form-grid">
      ...
      {form.status==="Waiting"&&<Field label="Waiting For" wide helper="填写正在等待谁或哪个团队反馈，会出现在等待看板。" tip="例如“小王确认数据口径”。"><input value={form.waitingFor||""} onChange={e=>f("waitingFor",e.target.value)} placeholder="例如：小王确认埋点口径"/></Field>}
    ...
    <div className="dialog-foot"><span>保存后会自动写入当前数据源</span><button className="primary" disabled={!form.title.trim()} onClick={()=>onSave({...form,actualHours:taskHours(form),completedAt:form.status==="Done"?(form.completedAt||todayISO()):undefined})}><Save size={15}/> 保存任务</button></div>
  </BaseDialog>
}
...
function MeetingDialogV2({open,meeting,data,onCreateProject,onOpenChange,onSave}:{open:boolean;meeting:Meeting|null;data:WorkData;onCreateProject:(p:Project)=>Project;onOpenChange:(o:boolean)=>void;onSave:(m:Meeting)=>void}) {
  ...
  useEffect(()=>{if(open){const m=meeting?{...meeting,durationMinutes:meeting.durationMinutes||0,attendees:[...meeting.attendees],decisions:[...meeting.decisions],actionItems:[...meeting.actionItems]}:blank();setForm(m);setActionRows(m.actionItems);setActionsText(serializeMeetingActions(m.actionItems))}},[open,meeting]);
  ...
  const submit=()=>onSave({...form,attendees:uniqueNames(form.attendees),actionItems:actionRows.filter(a=>a.text.trim()).map(a=>({id:a.id||uid("action"),text:a.text.trim(),owner:a.owner?.trim()||"我",dueDate:a.dueDate||todayISO(),taskId:a.taskId}))});
  ...
  return <BaseDialog open={open} onOpenChange={onOpenChange} title={meeting?"编辑会议":"新建会议"} subtitle="记录讨论、决策与可执行的行动项。" wide>
    <div className="form-grid">
      ...
      <Field label="日期与时间" helper="用于会议时间线、工作日志和报告统计。"><input type="datetime-local" value={form.date} onChange={e=>f("date",e.target.value)}/></Field>
