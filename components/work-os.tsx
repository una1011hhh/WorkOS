...
function WeeklyAnalytics({ data, weekStart, setWeekStart, onTask, onMeeting, onReflection }: { data: WorkData; weekStart: string; setWeekStart: (s: string) => void; onTask: (t: Task) => void; onMeeting: (m: Meeting) => void; onReflection: (r: Reflection) => void }) {
  ...
  return <div className="analytics-section">
    ...
    <section className="panel weekly-timeline"><PanelHead title="周工作时间轴" sub="根据真实计时、会议、复盘与等待记录生成" />{stats.events.length ? <>
      <div className="timeline-board">{days.map(day => { const date = format(day,"yyyy-MM-dd"), events = stats.events.filter(e=>e.date===date); return <div className="timeline-day" key={date}><div className="timeline-day-head"><b>{format(day,"EEE",{locale:zhCN}).toUpperCase()}</b><span>{format(day,"MM/dd")}</span></div><div className="timeline-lane">{events.map(e => <button key={e.id} title={`${e.kind}：${e.title} · ${durationLabel(e.durationSeconds)}`} onClick={()=>openEvent(e)} style={{ left: `${Math.min(88, Math.max(0, (e.startHour-8)/12*100))}%`, width: `${Math.min(72, Math.max(14, e.durationSeconds/3600/12*100))}%`, background: e.color }}><span>{e.kind}：{e.title}</span></button>)}</div></div> })}</div>
      <div className="timeline-mobile-list">{days.map(day => { const date = format(day,"yyyy-MM-dd"), events = stats.events.filter(e=>e.date===date); return <section className="timeline-mobile-day" key={date}><div><b>{format(day,"EEE",{locale:zhCN})}</b><span>{format(day,"MM/dd")}</span></div>{events.length?events.map(e=><button key={e.id} className="timeline-mobile-event" onClick={()=>openEvent(e)}><span style={{background:e.color}}>{e.kind}</span><strong>{e.title}</strong><small>{durationLabel(e.durationSeconds)} · {projectName(data.projects,e.projectId)}</small></button>):<p>无记录</p>}</section> })}</div>
    </> : <EmptyState icon={Timer} title="本周还没有可分析时间记录" text="开始任务计时或记录会议后，这里会出现时间轴。"/>}</section>
