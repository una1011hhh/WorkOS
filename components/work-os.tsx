...
type PlannerEvent = { id: string; title: string; start: number; end: number; kind: "meeting" | "planned"; detail: string; task?: Task };
...
function DailyPlanner({data,setData,onTask,notify}:{data:WorkData;setData:React.Dispatch<React.SetStateAction<WorkData>>;onTask:(task:Task)=>void;notify:(message:string)=>void}) {
  ...
  const events=[...meetingEvents,...plannedEvents].filter(event=>event.end>8*60&&event.start<20*60).sort((a,b)=>a.start-b.start||a.end-b.end);
  ...
  return <div className="planner-page">
    ...
    <div className="planner-layout">
      <section className="panel planner-calendar"><div className="planner-legend"><span className="meeting">会议</span><span className="planned">计划任务</span><small>左侧固定会议，右侧安排任务；同一时间可并行</small></div><div className="planner-track-head"><span>会议</span><span>计划任务</span></div><div className="planner-timeline"><div className="planner-hour-labels">{Array.from({length:13},(_,index)=><span key={index} style={{top:`${index*60}px`}}>{plannerTime((8+index)*60)}</span>)}</div><div className="planner-canvas">{Array.from({length:13},(_,index)=><i className="planner-grid-line" key={index} style={{top:`${index*60}px`}}/>)}<i className="planner-lane-divider"/>{events.map(event=>{const startMinute=Math.max(8*60,event.start),endMinute=Math.min(20*60,event.end);return <button key={event.id} className={`planner-event ${event.kind}`} style={{top:`${startMinute-8*60}px`,height:`${Math.max(24,endMinute-startMinute)}px`}} onClick={()=>event.task&&onTask(event.task)}><strong>{event.title}</strong><span>{plannerTime(event.start)}–{plannerTime(event.end)}</span><small>{event.detail}</small></button>})}</div></div></section>
